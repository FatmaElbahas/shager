...
async function loadActivities(page = 1, search = "") {
  ...
  try {
    const response = await fetch(
      `/api/tree_creator/request?page=${page}&search=${encodeURIComponent(
        search
      )}`,
      {
        headers: {
...
<td class="text-center">${activity.user_phone || "-"}</td>
...
// تأكيد تغيير الحالة
function confirmStatusChange(activityId, status) {
...
async function updateStatus(activityId, status) {
  try {
    const response = await fetch(
      `/api/tree_creator/request/${activityId}`,
      {
        method: "PUT",
        headers: {
    ...
    updateActivityRowStatus(activityId, status);
...
function updateActivityRowStatus(activityId, status) {
